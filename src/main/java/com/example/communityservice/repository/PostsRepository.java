...
/** Posts 엔티티 데이터베이스 접근 인터페이스 게시글 CRUD, 검색, 정렬, 통계 기능 제공 */
@Repository
public interface PostsRepository extends JpaRepository<Posts, Long> {
  ...
  /**
   * JOIN을 사용한 성능 최적화 쿼리 사용자가 좋아요한 게시글 목록을 한 번의 쿼리로 조회
   *
   * <p>일반적인 방식의 문제: - PostLikes에서 좋아요 목록 조회 → N번의 Posts 조회 (N+1 문제)
   *
   * <p>JOIN 쿼리 방식: - Posts와 PostLikes를 JOIN으로 한 번에 조회 → 1번의 쿼리 - 데이터베이스 레벨에서 페이징 처리로 메모리 효율성 확보
   *
   * @param userId 사용자 ID
   * @param pageable 페이징 정보
   * @return 좋아요한 게시글 목록 (페이징 처리됨)
   */
  // 특정 사용자가 좋아요한 게시글 목록 조회 (최신순) - 마이페이지용
  @Query(
